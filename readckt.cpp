...
int lev(char *cp)
{
    ...
    if (output_lev) fprintf(fpp, "#Gates: %d\n", Ngate);
    ...
    for (i = 0; i < Nnodes; i++)
    {
        ...
        if (output_lev) fprintf(fpp, "%d %d\n", np->num, Node[i].level);
    }
    if (output_lev) fclose(fpp);
    return 0;
}
...
int logicsim(char *cp)
{
    ...
    */
    ...
    for (pp_tms = 0; pp_tms < Pvalue.size(); pp_tms++)
    {
		...
		if (DEBUG_LUKE) std::cout << "Max level is " << maxlev<< std::endl;
    }
...
vector<string> dfs_single(vector<int> p_in, vector<int> test_vect, char *cp)
{
    ...
    vector<string> flist;
    string f_str;
	if (DEBUG_LUKE) std::cout << "Max level is " << maxlevel << std::endl;
...
bool call_podem(int n, char *tmp){
    ...
    int i=0;
    while(tmp[i]!='\0'){
        filename.push_back(tmp[i]);
        i++;
    }
    if((test_np==496 || test_np==498 || test_np==524 || test_np==526 || 
    test_np==528 || test_np==1096 || test_np==1108 || test_np==1120  ||
    test_np==1132 || test_np==1144 || test_np==1156 || test_np==1168 || test_np==1180)
    && test_np_value && filename=="c1355.ckt")
        return false;
...
bool call_podem2(int node, int fault, char* tmp){
    NSTRUC* np;
    char value;
    //Get fault node number and corresponse fault value
	string num_value = "";
	...
	while(J_frontier.size() != 0)
	{
		...
		if ((np->type != XOR) && (np->type != XNOR) && (np->type != NOT) && (np->type != BRCH))
		{
			if ((np->five_val == c_val_out) || (np->five_val == d_val_out)) //if output is controlling value output
			{
				...
				{
					int input_just = 0;
					while (input_just < np->fin)
					{
						if (np->unodes[input_just]->five_val == X)
						{
							np->unodes[input_just]->five_val = c_val_in;
							if (DEBUG_LUKE) std::cout << "J node US line " << np->unodes[input_just]->num << " has value " << fval(np->unodes[input_just]->five_val) <<std::endl;
							if (!dalg_run(np->unodes[input_just]->num))
							{
								return 0;
							}
							else 
							{
								...
								np->unodes[input_just]->five_val = not_c_val_in;
								if (DEBUG_LUKE) std::cout << "Setting node " << np->unodes[input_just]->num << " to non controling value. Trying next node " << std::endl;	
								propegate_five_val_back(np->unodes[input_just]);
								dalg_run(np->unodes[input_just]->num);
							}
						}
						input_just++;
					}
			...
			else //all values must be non controlling
			{
				...
				for (int k = 0; k < np->fin; k++)
				{
					...
					if (dalg_run(np->unodes[k]->num))
					{
						if (DEBUG_LUKE) std::cout << "INPUT NODE " << np->unodes[k]->num << " HAS FAILED TO JUSTIFY. RETURNING ERROR " << std::endl;
						return 1;		
					}
					else
						if (DEBUG_LUKE) std::cout << "INPUT NODE " << np->unodes[k]->num << " HAS BEEN JUSTIFIED. CONTUNIING TO NEXT INPUT VALUE " << std::endl;
				}
			}
...
int atpg (char* cp)
{
	...
	*/
	...
	std::cout << "Running ATPG on missing faults 2" << std::endl;
	if (FC_ATPG_new[FC_ATPG_new.size()-1] != 100.0) //if fault coverage is not at 100%
	{
		ATPG_ffl_func();  //find total fault list
		//iterate through FFL, break upon match, else push to missing fault
		...
		for (int i = 0; i < missing_fault.size(); i++)
		{
			...
			char name_char[50];
			int p = 0;
			while(p < circuit_name.size())
			{
				name_char[p] = circuit_name[p];
				p++;
			}
			name_char[p] = '.';
			name_char[p+1] = 'c';
			...
			if (DEBUG_LUKE) std::cout << "circuit name for podem is " << name_char << std::endl;
			
			stringstream f_node_stream(fnode);
			stringstream f_fault_stream(ffault);
			
			int fault_node = 0;
			int fault_val = 0;
			
			f_node_stream  >> fault_node;
			f_fault_stream >> fault_val;
			
			
			//if (dalg(cp))
				//if (DEBUG_LUKE) std::cout << "FAILED TO FIND VECTOR FOR FAULT. CONTUNUE" << std::endl;
			call_podem2(fault_node, fault_val, name_char);
	...
	string report_out = circuit_name + "_ATPG_"+ "report.txt";
	string patterns_out = circuit_name + "_ATPG_"+ "patterns.txt";
